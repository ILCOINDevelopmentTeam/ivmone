...
constexpr InstructionCategory get_instruction_category(ivmc_opcode opcode) noexcept
{
...
void register_synthetic_benchmarks()
{
    ...
    // PUSH.
    for (auto opcode = OP_PUSH1; opcode <= OP_PUSH32; opcode = static_cast<ivmc_opcode>(opcode + 1))
        params_list.insert(
            params_list.end(), {{opcode, Mode::min_stack}, {opcode, Mode::full_stack}});

    // SWAP.
    for (auto opcode = OP_SWAP1; opcode <= OP_SWAP16; opcode = static_cast<ivmc_opcode>(opcode + 1))
        params_list.insert(params_list.end(), {{opcode, Mode::min_stack}});

    // DUP.
    for (auto opcode = OP_DUP1; opcode <= OP_DUP16; opcode = static_cast<ivmc_opcode>(opcode + 1))
        params_list.insert(
            params_list.end(), {{opcode, Mode::min_stack}, {opcode, Mode::full_stack}});
    ...
    for (auto& [vm_name, vm] : registered_vms)
    {
        RegisterBenchmark((std::string{vm_name} + "/total/synth/loop_v1").c_str(),
            [&vm = vm](State& state) { bench_ivmc_execute(state, vm, generate_loop_v1({})); });
        RegisterBenchmark((std::string{vm_name} + "/total/synth/loop_v2").c_str(),
            [&vm = vm](State& state) { bench_ivmc_execute(state, vm, generate_loop_v2({})); });
    }
    ...
    for (const auto params : params_list)
    {
        for (auto& [vm_name, vm] : registered_vms)
        {
            RegisterBenchmark((std::string{vm_name} + "/total/synth/" + to_string(params)).c_str(),
                [&vm = vm, params](
                    State& state) { bench_ivmc_execute(state, vm, generate_code(params)); })
                ->Unit(kMicrosecond);
        }
    }
}
